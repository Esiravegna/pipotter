...
import json
from sfx.lights.effect import LightEffect, SFXError  # Assuming your code is in light_effect.py
...
def test_valid_json_config(mock_neopixel, mock_board, mock_pixel_order):
    ...
    mock_neopixel.assert_called_once_with(18, 16, brightness=0.5, auto_write=False, pixel_order='GRB')
    
    # Run the effect (will use the mock)
    effect.run()
    assert mock_neopixel.return_value.show.call_count > 0, "Expected show() to be called"
